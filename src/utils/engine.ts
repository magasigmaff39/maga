...
export function matchUniversities(profile: ApplicantProfile): University[] {
  ...
  const scored = UNIVERSITY_DATABASE.map(uni => {
    ...
    // Olympiad bonus
    if (profile.olympiadLevel === 'republican' || profile.olympiadLevel === 'international') {
      if (uni.id === 'nu' || uni.id === 'kaist' || uni.id === 'hkust' || uni.id === 'aitu') {
        score += 15;
      }
    }
    ...
    // Determine Fit Tier (Dream / Target / Safety) dynamically
    let fitTier: FitTier = 'Target';
    if (uni.acceptanceRate && parseInt(uni.acceptanceRate) <= 15) {
      fitTier = 'Dream';
    } else if (uni.minGpa > normGpa || (uni.minSat && userSat > 0 && userSat < uni.minSat)) {
      fitTier = 'Dream';
    } else if (normGpa >= uni.minGpa + 0.3 && userIelts >= uni.minIelts + 0.5 && (parseInt(uni.acceptanceRate || '30') >= 30)) {
      fitTier = 'Safety';
    } else {
      fitTier = 'Target';
    }
...
export function generateRoadmap(profile: ApplicantProfile, matchedUnis: University[]): RoadmapStep[] {
  ...
  const steps: RoadmapStep[] = [
    ...
    },
    {
      ...
      subtasks: [
        { id: 'dl-1', title: 'Подать заявку на грантовую программу Stipendium Hungaricum (дедлайн середина января)', isCompleted: false },
