    ...
    <div className="space-y-8 py-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-200">
        ...
        <div className="self-start sm:self-auto px-3 py-1 rounded-lg bg-slate-100 border border-slate-200 text-xs font-semibold text-slate-800">
          {t('Фокус стратегии')}: {tx(recommendedCategoryFocus)}
        </div>
      </div>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 pb-6 border-b border-slate-100">
          ...
          <div className="flex gap-3 w-full md:w-auto">
            <div className="flex-1 md:w-36 p-3 rounded-xl bg-slate-50 border border-slate-200 text-center">
              <span className="text-[10px] font-semibold text-slate-400 block uppercase">{t('Статус')}</span>
              <span className="text-xs font-bold text-slate-900 block mt-0.5">
                {t(profile.grade === 'grade_11' ? '11 класс' : '10 класс')}
              </span>
            </div>
            <div className="flex-1 md:w-36 p-3 rounded-xl bg-slate-50 border border-slate-200 text-center">
              <span className="text-[10px] font-semibold text-slate-400 block uppercase">{t('Набор')}</span>
              <span className="text-xs font-bold text-slate-900 block mt-0.5">
                {profile.targetYear}
              </span>
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="p-4 rounded-xl bg-slate-50 border border-slate-100 space-y-2">
            ...
            <p className="text-[11px] text-slate-500">
              GPA {profile.gpa} • {profile.hasSat ? `SAT ${profile.satScore}` : profile.hasUnt ? `ЕНТ ${profile.untScore}` : t('Тесты в процессе')}
            </p>
          </div>
          ...
          <div className="p-4 rounded-xl bg-slate-50 border border-slate-100 space-y-2">
            ...
            <p className="text-[11px] text-slate-500">
              {profile.hasIelts ? `IELTS ${profile.ieltsScore} Band` : t('Сертификат отсутствует')}
            </p>
          </div>
          ...
          <div className="p-4 rounded-xl bg-slate-50 border border-slate-100 space-y-2">
            ...
            <p className="text-[11px] text-slate-500">
              {profile.olympiadLevel !== 'none' ? `${t('Олимпиада')}: ${profile.olympiadLevel}` : t('Базовое портфолио')}
            </p>
          </div>
          ...
          <div className="p-4 rounded-xl bg-slate-50 border border-slate-100 space-y-2">
            ...
            <p className="text-[11px] text-slate-500">
              {t(profile.budgetTier === 'grant_only' ? '100% Гранты и стипендии' : 'Частичное софинансирование')}
            </p>
          </div>
            ...
            {bottlenecks.map((item, idx) => (
              ...
              >
                ...
                <div className="pt-1 text-[11px] font-medium text-slate-800 flex items-start gap-1 bg-white p-2.5 rounded-lg border border-slate-200">
                  <span className="font-semibold text-slate-900">{t('Регламент:')}</span>
      ...
      <div className="p-6 rounded-2xl bg-slate-900 text-white shadow-xs space-y-2">
        <div className="flex items-center gap-2 text-xs font-bold text-slate-400 uppercase tracking-wider">
          ...
          <span>{t('Экспертное заключение по профилю')}</span>
        </div>
