...
import { Compass, FileText, Calendar, RotateCcw, Mail, User, LogOut, Moon, Sun, Menu, X } from 'lucide-react';
    ...
    <header className="sticky top-0 z-40 border-b border-slate-200/80 dark:border-zinc-800 bg-white/85 dark:bg-zinc-950/85 backdrop-blur-xl transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 gap-3">
          <button type="button" className="flex items-center gap-2.5 select-none rounded-2xl" onClick={onReset}>
          ...
          <div className="hidden lg:flex items-center gap-2">
            ...
            <button
              ...
              className="ar-btn flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-medium text-slate-700 hover:bg-slate-100 border border-slate-200 dark:text-zinc-200 dark:border-zinc-700 dark:hover:bg-zinc-800"
            >
          ...
          <div className="flex items-center gap-1.5">
            ...
            {currentUser ? (
              <div className="hidden sm:flex items-center gap-1.5">
                <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-slate-100 border border-slate-200 text-xs dark:bg-zinc-800 dark:border-zinc-700 shadow-sm">
                  ...
                  <span className="font-semibold text-slate-900 dark:text-white max-w-[90px] truncate">
                    {currentUser.firstName}
                  </span>
                </div>
                <button type="button" onClick={onLogout} className="p-2 text-slate-400 hover:text-rose-600 rounded-xl hover:bg-slate-100 dark:hover:bg-zinc-800" title="Выйти">
              ...
              <button
                type="button"
                onClick={onOpenAuth}
                className="ar-btn hidden sm:inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl text-xs font-semibold bg-slate-900 text-white hover:bg-slate-800 dark:bg-white dark:text-zinc-900"
              >
            ...
            <button type="button" onClick={onReset} className="p-2 text-slate-400 hover:text-slate-700 rounded-xl hover:bg-slate-100 dark:hover:bg-zinc-800" title="Сбросить">
              <RotateCcw className="w-4 h-4" />
            </button>

            <button type="button" className="lg:hidden p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-zinc-800" onClick={() => setMenuOpen((v) => !v)}>
        ...
        {menuOpen && (
          <div className="lg:hidden pb-4 flex flex-col gap-2 animate-fadeIn">
            <button type="button" onClick={() => { onOpenEmailModal(); setMenuOpen(false); }} className="ar-btn ar-btn-ghost w-full justify-start">План на почту</button>
