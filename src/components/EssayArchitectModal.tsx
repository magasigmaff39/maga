      ...
      <div className="bg-white w-full max-w-3xl rounded-2xl border border-slate-200 shadow-xl overflow-hidden flex flex-col max-h-[90vh]">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-sm font-bold text-slate-900">
                {t('Модульная структура мотивационного письма')}
              </h3>
              <p className="text-[11px] text-slate-500">
                {t('Академический регламент составления Personal Statement')}
              </p>
            </div>
          </div>
          ...
          {activePart === 1 && (
            <div className="space-y-3">
              <div className="flex items-start gap-2 p-3 bg-slate-50 rounded-xl border border-slate-200 text-xs text-slate-700">
                ...
                <span>
                  <strong>{t('Методическое указание')}:</strong> {t('Избегайте шаблонных формулировок. Опишите конкретную проблему, аналитический кейс или задачу, которая инициировала ваш интерес к направлению.')}
                </span>
              </div>
              <label className="block text-xs font-semibold text-slate-700">
                {t('Вводный контекст и мотивационный импульс:')}
              </label>
              <textarea
                rows={5}
                value={hookText}
          ...
          {activePart === 2 && (
            <div className="space-y-3">
              <div className="flex items-start gap-2 p-3 bg-slate-50 rounded-xl border border-slate-200 text-xs text-slate-700">
                <Lightbulb className="w-4 h-4 text-slate-600 shrink-0 mt-0.5" />
                <span>
                  <strong>{t('Методическое указание')}:</strong> {t('Приведите 1-2 подтвержденных проекта или олимпиадных кейса. Опишите поставленную цель, примененный метод и измеримый результат.')}
                </span>
              </div>
              <label className="block text-xs font-semibold text-slate-700">
                {t('Академический бэкграунд и проектная деятельность:')}
              </label>
              <textarea
                rows={5}
                value={academicText}
          ...
          {activePart === 3 && (
            <div className="space-y-3">
              <div className="flex items-start gap-2 p-3 bg-slate-50 rounded-xl border border-slate-200 text-xs text-slate-700">
                <Lightbulb className="w-4 h-4 text-slate-600 shrink-0 mt-0.5" />
                <span>
                  <strong>{t('Методическое указание')}:</strong> {t('Укажите конкретные кафедры, лаборатории или академические программы выбранного университета.')}
                </span>
              </div>
              <label className="block text-xs font-semibold text-slate-700">
                {t('Обоснование соответствия профилю университета:')}
              </label>
              <textarea
                rows={5}
                value={whyUniText}
          ...
          {activePart === 4 && (
            <div className="space-y-3">
              <div className="flex items-start gap-2 p-3 bg-slate-50 rounded-xl border border-slate-200 text-xs text-slate-700">
                <Lightbulb className="w-4 h-4 text-slate-600 shrink-0 mt-0.5" />
                <span>
                  <strong>{t('Методическое указание')}:</strong> {t('Сформулируйте профессиональные цели и вклад в развитие индустрии после завершения обучения.')}
                </span>
              </div>
              <label className="block text-xs font-semibold text-slate-700">
                {t('Профессиональные перспективы и прикладной импакт:')}
              </label>
              <textarea
                rows={5}
                value={futureText}
        ...
        <div className="px-6 py-4 border-t border-slate-200 bg-slate-50 flex items-center justify-between">
          ...
          >
            ...
            <span>{t(copied ? 'Скопировано в буфер' : 'Скопировать структуру эссе')}</span>
          </button>
